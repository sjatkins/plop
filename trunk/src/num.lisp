...
(defun aa-computable-p (expr)
  (and (consp expr)
       (matches (fn expr)
		(if impulse log exp sin cos + * order let / sqrt sqr -))))
...
;; 		      (return (aa-widen (reduce #'aa-* aas) 0.0)))
      ...
      (- (if (aa-unreal-p aa0)
	     (return)
	     (progn (assert (eql (length args) 1))
		    (aa-* (make-aa -1) aa0))))
      (sin (if (aa-unreal-p aa0)
	       (return)
	       (aa-sin aa0)))
      (cos (if (aa-unreal-p aa0)
	       (return)
	       (aa-sin (aa-+ aa0 (make-aa half-pi)))))
      ...
      (/ (if (some #'aa-unreal-p aas)
		 ...
		 (aa-inv aa0)
		 (aa-/ aa0 (cadr aas)))))
      (sqrt (if (or (aa-unreal-p aa0) (< 0 (aa-min aa0)))
		(return)
		(aa-sqrt aa0)))
      (sqr (if (aa-unreal-p aa0)
	       (return)
	       (aa-expt aa0 2)))
      (order (car aas))
...
;;; xs should be a list of arg lists (e.g. ((1) (2) (3)) if expr takes 1 num arg
(defun linear-scale (argnames expr xs ys)
  (mvbind (a b) 
      (linear-regress ys (mapcar (lambda (x)
				   (aprog1 (with-bound-values *empty-context*
					       argnames x (peval expr))
				     (unless (and (finitep it) (realp it))
				       (return-from linear-scale))))
				 xs)
		      +aa-precision+)
    (plist '+ a (plist '* b expr))))
