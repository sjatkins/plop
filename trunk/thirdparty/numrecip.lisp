;;;-*- Mode: Lisp; Package: NUMRECIP -*-
;; note: I have commented out some functions that seem suspicious, and cleaned
;; up a bit so that sbcl compiles without warnings
;; still, use at your own risk...
;; Moshe Looks (madscience@google.com) 4/13/2009

(cl:defpackage #:numrecip
  (:use #:common-lisp)
  (:export #:ludcmp
	   #:lubksb
	   #:polint
...
(defun ran3 (idum)			;uniform random deviate between 0.0 amd 1.0
  ...
  (progn				;original uses a VAR (reference) parameter, hence rseed here
...
(defun gasdev (idum)			;gaussian noise, zero mean, unit variance, uses ran3
  (cond ((zerop GasdevIset)		;no extra number handy -- go calulate two and save one for next time
...
(defun zroots (a m roots polish)
  (let ((eps 2.0e-6)
	(ad (make-array (+ m 2) :element-type 'complex :initial-element #C(0 0)))
	(b #C(0 0))
	(c #C(0 0))
	(dum 0.0))
...
(defun correl (data1 data2 n ans)	;compute correlation of data1 and data2.  Return result in ANS.
  (let ((fft (make-array (+ (* n 2) 1) :element-type 'float :initial-element 0.0))
	(no2 0) (ii 0) (dum 0.0))
...
(defun fixrts (d npoles)
  (let ((a (make-array (+ npoles 2) :element-type 'complex :initial-element #C(0 0)))
	(roots (make-array (+ npoles 2) :element-type 'complex :initial-element #C(0 0)))
	(dum 0.0))
...
;;   (let ((reg (make-array (1+ npoles) :element-type 'float :initial-element 0.0)))
;;     (loop for j from 1 to npoles do
