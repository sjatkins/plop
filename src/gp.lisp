...
(defun gp-regress (cost &optional (popsize 4000)
		   (target (lambda (x) (* x (1+ (* x (1+ (* x (1+ x))))))))
		   (sample (generate 20 (lambda () (list (1- (random 2.0)))))))
  (setf sample (mapcar (lambda (x) (if (numberp x) (list x) x)) sample))
  (let* ((argtypes (ntimes (length (car sample)) num))
	 (argnames (make-arg-names argtypes))
	 (scorers (mapcar (lambda (x)
    ...
    (setf (car scorers) (lambda (x) (incf at) (funcall first x)))
