  ...
  (unless (atom expr) 
    (acase (fn expr)
      ...
      (split (let ((ttypes (mapcar (bind #'expr-type /1 context) 
	       ...
	       (assert (every #'eq (mapcar #'cadadr ttypes)
			      (mapcar #'caddr ttypes)))
	       (cons (list 'func (mappend #'cdr ttypes) type) ttypes)))
...
(define-test arg-types
  (assert-equal '((func ((list num) num) bool) (tuple (list num) num))
		(arg-types %(split (lambda (x y) (< (car x) y))
				   (tuple (list 1 2) 3))
			   *empty-context* bool))
  (assert-equal '((func ((list num) num (list bool) bool) bool) 
...
(defun arg-types-p (expr context &aux (fn (fn expr)))
  ...
  (or (matches fn (impulse 0< < list split lambda tuple if let)) (closurep fn) 
      (typedp fn context)))
